...
def run5(candles, pre, g, entry_filter=None, events=None, funding=None):
    ...
    def float_mark(p, px_now):
        ...
        Когда это дно СРАБАТЫВАЕТ (проверялось счётчиком: на боевых прогонах
        SOL x5, DOGE x10 и DOGE x25 — ни разу, при четырёх ликвидациях). Так и
        должно быть: ликвидация проверяется РАНЬШЕ любой переоценки. Пока цикл
        жив, бар не достал ни стопа, ни цены ликвидации, а она наступает выше
        банкротной — то есть до «минуса размером во всю маржу» цена не дошла и
        дно недостижимо. Доливки внутри бара только опускают среднюю, и вместе
        с ней банкротную цену, так что порядок не ломают.

        Достижимый путь ровно один: цикл, ОТКРЫТЫЙ НА ПОСЛЕДНЕЙ свече данных.
        Его закрывает принудительный close_pos после цикла по барам, и
        переоценка идёт по лоу того же бара — единственного, который
        ликвидационная проверка не смотрела (на момент входа он уже прошёл, и
        сам этот лоу случился ДО входа: цена берётся по закрытию). Без дна
        такой цикл рисует минус больше всей изолированной маржи — величину,
        которой на счёте не бывает. Поэтому дно оставлено: это не защита «на
        всякий случай», а единственное, что удерживает искусственный хвост
        прогона в границах реального риска. Закреплено тестом (test_gridlib,
        раздел 9h): убрать дно — тест падает.
        """
    ...
    for i in range(start_i, len(candles)):
        ...
        if pos:
            ...
            # ФАНДИНГ ЗА СВЕЧУ УДЕРЖАНИЯ. ЗНАК ЗАВИСИТ ОТ СТОРОНЫ.
            #
            # ЗДЕСЬ БЫЛА ОШИБКА, и она держалась долго. Стояло
            #     pos["fees"] += q_pre * c * FUND_8H / BARS_8H
            # то есть фандинг СПИСЫВАЛСЯ ВСЕГДА, какой бы ни была сторона.
            # На бирже это не так: при ПОЛОЖИТЕЛЬНОЙ ставке лонги платят
            # шортам, при отрицательной наоборот. У LTC ставка положительна
            # в 83.5% случаев, а лучший конфиг проекта (LTC/final_w) на 99%
            # в шорте — значит движок брал с него плату, которую он на самом
            # деле ПОЛУЧАЛ БЫ.
            #
            # Ошибка смещала результат в безопасную сторону (числа занижены),
            # но она перекашивала и СРАВНЕНИЕ конфигов между собой: шортовые
            # штрафовались, лонговые считались верно. А по этому сравнению
            # выбирались боты для витрины.
            #
            # sgn здесь уже +1 для лонга и -1 для шорта, поэтому знак выходит
            # сам собой: лонг при ставке > 0 платит, шорт при ставке > 0
            # получает.
            #
            # funding — необязательный ряд НАСТОЯЩИХ ставок ПО БАРАМ, В ДОЛЯХ
            # (не в процентах: в файлах проекта они в процентах, делить на 100
            # обязан вызывающий). Не передан — берётся плоская FUND_8H, но уже
            # с верным знаком.
