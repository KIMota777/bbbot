...
Замер кэшей на 14.08.2026 (`python ext_data.py`):
    ...
    funding  — с 2020-10-26 (6400 точек по 8ч), окно закрывает целиком;
    OI       — с 2020-07..2021-06 в зависимости от монеты (45-53 тысячи
               часов), окно закрывает целиком. ДО 29.08.2026 ряд начинался с
               2024-10-16 и покрывал лишь 57.9% окна — но это было
               ограничение НАШЕЙ пагинации (80 страниц), а не биржи: прямой
               запрос показал, что Bybit отдаёт открытый интерес и за февраль
               2021. Лимит поднят до 300 страниц;
...
Замер покрытия на боевом окне (15m, 1150 дней = 3.15 года, замер
29.08.2026, после углубления рядов):

    ряд        всё окно   обучение+валидация   экзамен
    fund         100.0%               100.0%    100.0%
    spx5         100.0%               100.0%    100.0%
    dxy5         100.0%               100.0%    100.0%
    oi_chg       100.0%               100.0%    100.0%
    gold5         42.8%                26.8%    100.0%

ЧТО ЭТО ПОМЕНЯЛО. Ген oi_gate раньше срабатывал лишь на 46.1% той части
окна, по которой шёл ОТБОР, — то есть половины сравнивались не на равных.
С полным рядом обучающая половина LTC/final_w падает с +114.8% до +99.7%
и теряет 41 сделку из 258: фильтр наконец работает и там. Холдоут не
изменился — он и раньше был покрыт целиком. Золото по-прежнему неполно,
но это фильтр ЛОНГА, а живые конфиги почти всегда в шорте.
...
def fetch_oi(symbol):
    ...
    ГЛУБИНА БЫЛА ОГРАНИЧЕНА НАШИМ КОДОМ, А НЕ БИРЖЕЙ. Здесь стояло 80
    страниц по 200 точек = 16000 часов ≈ 667 дней, и в докстроке было
    написано, что «дальше выборка просто обрывается». Проверка 29.08.2026
    прямым запросом к Bybit показала обратное: открытый интерес по LTCUSDT
    отдаётся и за февраль 2021, то есть на всю доступную историю свечей.

    Чем это было плохо. Ряд начинался с 2024-10-16, покрывая 57.9% окна
    бэктеста и лишь 46.1% той части, по которой шёл ОТБОР (теперь 100%). Ген oi_gate на
    остальных свечах не срабатывал вовсе. При этом гейт решает много: у
    LTC/final_w его выключение меняет холдоут с +39.3% на -5.0%, а просадку
    с 9.7% на 27.4%. То есть ключевой фильтр работал на половине обучающей
    половины и на всём холдоуте — половины сравнивались не на равных.

    Теперь страниц хватает на всю историю (48000 часов ≈ 2000 дней), и
    выборка останавливается сама, когда биржа перестаёт отдавать данные.
    Сейчас ген включён у BTC, LTC и DOGE
    ...
    def build():
        s = HTTP(testnet=False)
        out, cur = [], None
        # 300 страниц по 200 часов = 60000 часов ≈ 2500 дней: хватает на всю
        # историю контракта. Выборка всё равно остановится раньше — как
        # только биржа перестанет отдавать страницы.
        for _ in range(300):
