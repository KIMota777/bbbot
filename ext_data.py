...
вызывающего оно не должно: fetch_* зовут 28 модулей репозитория (замер
29.08.2026 по grep, без тестов), в том числе webapp/app.py, где исключение —
...
все 28 продакшен-вызовов fetch_daily_pct5 (замер счётом по исходникам
29.08.2026, включая сайт и бота) идут без аргумента, а bot_rsi.get_macro
...
class MacroUnavailable(RuntimeError):
    ...
    Наружу не выходит. Так сделано намеренно: из 28 вызывающих исключение
    ловит РОВНО ОДИН (bot_rsi.py, метод get_macro) — остальные двадцать семь,
...
def fetch_daily_pct5(max_age_s=None):
    ...
    Замер счётом по исходникам: 28 продакшен-вызовов fetch_daily_pct5 (список —
    bot_rsi, webapp/app, report3y, build_analytics, build_pnl_curves,
    check_stop_rate, debug_eth_4h, finalize_final_bots, improve_v13 и восемь
    evolution*), и ВСЕ СЕМНАДЦАТЬ зовут её без аргумента. Вызовы с max_age_s —
