...
ПРАВИЛА ЧЕСТНОСТИ (нарушение = брак, тест их проверяет)
  ...
  6. ОТРИЦАТЕЛЬНЫЙ РЕЗУЛЬТАТ — ВАЛИДНЫЙ. Вердикт «не подтверждён» это ответ,
...
RESULT_DOC = {
    ...
    "train": "метрики на [0..72%): n, wins, wr, wr_breakeven, exp_r, exp_usd, "
...
VERDICT_RULES = [
    f"«работает» — пройдены ВСЕ восемь критериев: сделок на холдоуте "
    ...
    f"одной сделки <= {MAX_ABS_R}",
    "три последних критерия добавлены после аудита: без них «работает» "
    "получали 2.8% чисто случайных сигналов, а сетап, отобранный перебором "
    "4320 вариантов по холдоуту (обучение -75.8%, холдоут +121.8%), "
    "проходил все проверки",
...
def evaluate(spec, tried=None, n_ctrl=N_CTRL_SEEDS, n_pert=N_PERT,
    ...
    if status == "работает":
        reasons.append("все пять критериев пройдены; это НЕ гарантия будущего "
                       "— это отсутствие явных признаков подгонки")
