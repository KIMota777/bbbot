...
server = MCPServer(
    ...
    instructions=(
        ...
        "   что получилось и как это повлияет на ботов.\n\n"
...
@server.tool(
    ...
    description=(
        "Принимает оценённые новости и обновляет новостной фон ботов. "
...
@server.tool(
    ...
    description=(
        ...
        "Если фон помечен stale, боты работают так, будто новостей нет."),
    annotations=RO)
...
def news_preview_effect(symbol: str, side: str = "L") -> dict:
    ...
    return {
        ...
        "примечание": (
            "Если реакция_включена = false, фон сейчас ни на что не влияет: "
            "коэффициенты news_* в config.py равны нулю. Это состояние по "
            "умолчанию — новостные гены не проходили walk-forward-отбор, "
            "включать их следует осознанно и сперва в DRY_RUN."),
    }
...
@server.tool(
    ...
    description=(
        ...
        "Удаление необратимо, но безопасно: пустой фон = боты работают как "
        "раньше, без новостей."),
