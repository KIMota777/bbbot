...
def base_runs(prep):
    ...
    for sym, d in prep.items():
        ...
        r = bh.run_at(d["candles"], d["pre"], d["g"], d["filt"], d["lev"],
                      events=evs)
        d["full"] = bh.summarize(
            r, evs, (d["candles"][-1][0] - d["candles"][0][0]) / (30 * 86400000))
...
def verdict(d, rob, rand_rets, rand_rank):
    """Тот же критерий, что в bots_honest: 4 пункта, объявлены до подсчёта."""
    ret = d["hold"]["m"]["ret"]
...
def main():
    ...
    for sym in syms:
        ...
        rand_rank = sum(1 for x in rand_rets if x < d["hold"]["m"]["ret"]) \
            / len(rand_rets) * 100
        rob, by_seed = robustness(d)
        c_tr, c_ho = costs(d, "train"), costs(d, "hold")
        lad, rec = ladder(d)
        status, okn, reasons = verdict(d, rob, rand_rets, rand_rank)
        med_p = statistics.median(rob)
        ...
        bots[sym] = dict(
            ...
            verdict=dict(status=status, key=STATUS_KEY[status],
                         score=f"{okn}/4", n_ok=okn, reasons=reasons,
                         notes=notes_for(d, rob, c_ho)),
