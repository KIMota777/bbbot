...
def neighbour_median(g, genes, clamp, evaluate, k=NEIGH_K, scale=NEIGH_SCALE,
    ...
    ЗАМЕР. Геном — боевой BTC: config.SYMBOL_PARAMS["BTCUSDT"]["final"],
    прогнанный через e7.cfg_to_genome + OFF12 + clamp, то есть ровно так, как
    его строит evolution12.run_symbol. Пространство GENES12 (46 генов, из них
    целых 26), seed=SEED, k=8, слотов 8*26=208. Шаг — попытка сдвига, «дошло» —
    сдвиг, переживший clamp:
      * без форс-шага          — 47 шагов, дошли все 47; 19 генов из 26 не
                                 двигались ни разу (узкие целые);
      * форс-шаг без разворота — 77 шагов, дошло 68 (9 срезал clamp), 8 генов
                                 из 26 не двигались ни разу;
      * форс-шаг с разворотом  — 77 шагов, дошли все 77, не двигались 4.
    Двухзначные гены-переключатели у боевого BTC стоят НА ГРАНИЦЕ, но не все на
    нуле: be_move=1, regime_gate=1, adx_gate=0, grid_mode=0. Доля соседей,
    которые ген переключают, при sigma=0.08 (замер по 1000 соседей на ген):
    без разворота 3.6% для гена на 1 и 4.1% для гена на 0 вместо обещанных 8% —
    примерно вдвое меньше, потому что половина шагов уходила наружу и
    срезалась. С разворотом стало 7.7% у всех четырёх: остаток до 8% —
    округление гауссова шага, не потеря.

    ЧИСЛА ПРЕДЫДУЩЕЙ РЕДАКЦИИ ЭТОЙ ДОКСТРОКИ (47/20, 77/41, 20 генов из 26)
    получены НЕ на боевом геноме, хотя он был заявлен: замерявший скрипт брал
    dict(config.SYMBOL_PARAMS["BTCUSDT"]) — а это словарь РЕЖИМОВ
    (normal/turbo/bear/final), не плоский геном, — и каждый ген в нём падал на
    GENES12[k][0], нижнюю границу. Мерили геном «всё по нижней границе», где
    clamp режет вдвое больше. Числа воспроизводятся, но описывают не то, что
    написано, поэтому заменены.

    ВОЗРАЖЕНИЕ ВТОРОГО КРУГА («7 дискретных сдвигов из 800, 16 генов из 20 не
    двигались ни разу») НЕ ПОДТВЕРДИЛОСЬ. Откуда его знаменатель: GENES4 — это
    20 генов, из них целых 8, и при k=40 выходит 40*20=800 слотов всего и
    40*8=320 целых. На базе, с которой стартует сама волна v4
    (e4.CURRENT["BTCUSDT"] + OFF4, пространство GENES4, seed=SEED, k=40):
      * до форс-шага   — 147 сдвигов из 320, без движения 3 гена из 8;
      * нынешний код   — 183 сдвига из 320, без движения 0 генов из 8.
    (На боевом геноме из config, свёрнутом в то же пространство GENES4, те же
    147 и 183 шага, дошло 146 и 183, без движения 4 гена из 8 и 0.)
    147, а не 7. И «16 генов из 20 не двигались» невозможно по устройству
    схемы: 12 из 20 генов GENES4 непрерывные, их сдвигает гауссов шаг всегда,
    и неподвижными могут оказаться только целые — их всего 8. Похоже, в
    возражении целые сдвиги поделены на общее число слотов, а непрерывные гены
    посчитаны как «не двигавшиеся». Направление мысли верное (узкие целые гены
    проверяются хуже непрерывных — ради этого и форс-шаг), но числа неверны.
    Все числа этой докстроки пересчитываются скриптом verify_neighbours4.py
    (scratchpad четвёртого круга): он печатает обе таблицы — на боевом геноме
    и на «нижней границе», — чтобы подмену геномов было видно сразу.
