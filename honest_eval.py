...
def neighbour_median(g, genes, clamp, evaluate, k=NEIGH_K, scale=NEIGH_SCALE,
    ...
    ЗАМЕР (боевой геном BTC из config.SYMBOL_PARAMS, пространство GENES12,
    seed=SEED, k=8; целых генов 26, слотов 8*26=208). Шаг — попытка сдвига,
    «дошло» — сдвиг, переживший clamp:
      * без форс-шага        — 47 шагов, дошло 20; 20 генов из 26 не
                               двигались ни разу (все узкие);
      * форс-шаг без разворота — 77 шагов, дошло 41 (36 срезал clamp), 9 генов
                               из 26 не двигались ни разу;
      * форс-шаг с разворотом — 77 шагов, дошли все 77, не двигались 4.
    Для двухзначного гена НА ГРАНИЦЕ (be_move, regime_gate, adx_gate,
    grid_mode — все четыре стоят на 0) доля соседей, которые его переключают,
    при sigma=0.08 составляла 4.1% вместо обещанных 8% (замер по 1000
    соседей на каждый ген) — ровно вдвое меньше, потому что половина шагов
    уходила наружу и срезалась. С разворотом стало 7.7%: остаток до 8% —
    ...
    40*8=320 целых. На этом самом замере (боевой геном BTC, GENES4, seed=SEED,
    k=40) получается:
    ...
    Все числа этой докстроки пересчитываются скриптом verify_neighbours.py
    (scratchpad третьего круга).
