...
def load_funding(symbol):
    """Ставка фандинга: (моменты расчёта, ставки В ДОЛЯХ). Раз в 8 часов.

    ВНИМАНИЕ НА ЕДИНИЦЫ. В файлах funding_*.json ставка лежит В ПРОЦЕНТАХ:
    ext_data.fetch_funding умножает ответ биржи на 100 (ext_data.py:465).
    Биржа отдаёт долю: 0.0001 это 0.01% за восемь часов. Значит число 0.01 в
    файле означает 0.01%, то есть долю 0.0001.

    Здесь ставка приводится к долям делением на 100 — ровно один раз и в
    одном месте. Без этого деления расход по фандингу завышался в сто раз:
    вместо примерно 7% годовых от объёма позиции получалось 733%, лонги
    платили эту сумму, а шорты её получали. Ошибка такого рода не портит
    точность, она подменяет победителя: в лидеры выходит любая стратегия с
    перекосом в короткую сторону, и по причине, не имеющей отношения к рынку.
    """
    key = ("fund", symbol)
    if key in _CACHE:
        return _CACHE[key]
    ...
    v = np.array([r[1] for r in rows], dtype=np.float64) / 100.0
    _CACHE[key] = (t, v)
    return t, v
