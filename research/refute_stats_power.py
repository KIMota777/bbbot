...
def main():
    ...
    print("\n   СВЯЗЬ «обучение -> экзамен» ВНУТРИ одной клетки (одна и та же")
    print("   стратегия, один и тот же настоящий перевес, разные истории):")
    for k in ("пусто|0.0000", "плато|0.0010", "плато|0.0030", "ровный|0.0030"):
        if k in results:
            print("      %-16s %+.2f" % (k.replace("|", " "), results[k]["corr"]))
    print("   Читать это надо так: когда истинный перевес у всех сравниваемых")
    print("   ОДИНАКОВ, связь «там хорошо — тут хорошо» равна нулю ПО")
    print("   ПОСТРОЕНИЮ, каким бы крупным этот перевес ни был. Отрицательная")
    print("   связь у автора означает ровно то же самое: его двадцать")
    print("   кандидатов различались между собой не перевесом, а удачей.")
    print("   Насколько велика была бы связь при РАЗНЫХ истинных перевесах —")
    print("   считает refute_stats_corr.py (ответ: около +0.06).")
