...
def build_filter(part, g):
    """Фильтр входа ДЛЯ ЭТОГО генома — строится всегда заново.

    ЗДЕСЬ БЫЛА ЧЕТВЁРТАЯ ОШИБКА ИЗМЕРЕНИЯ, и она обесценивала главный
    показатель отбора. Фильтр собирался ОДИН раз в halves() по исходному
    геному и лежал в part. Проверка устойчивости возмущает геном и зовёт
    one_half с СОСЕДОМ — а фильтр брала готовым из part, то есть от
    ИСХОДНОГО конфига. Значит гены, живущие в фильтре, не возмущались
    никогда: пороги фандинга, гейт открытого интереса, макро-пороги
    (spx/dxy/gold), EMA/MA/Aroon, режимный гейт, направление, паттерны,
    SMC (ob/fvg/structure) и гейт волатильности. Возмущалось только ядро
    RSI-сетки, то есть примерно половина генома.

    Проверяется это одним опытом: геном final_w, прогнанный на part от
    final_wf, отдавал числа final_wf (+104.68 / +56.61) вместо своих
    (+99.66 / +39.26). Ровно на этом стоял уровень A у LTCUSDT/final_wf:
    весь его отрыв от родителя — один ген fund_short_min, и он в фильтре,
    то есть ровно в непроверяемой половине.

    Стоит это дёшево: все make_filter* лишь замыкают уже посчитанные ряды
    aux, а regimes для гейта волатильности считается один раз и живёт в part.

    ЧЕГО ЭТО НЕ ЧИНИТ. bots_honest.perturb двигает только NUMERIC_GENES;
    vol_gate в этот список не входит и здесь по-прежнему не возмущается —
...
def halves(sym, g, pct5):
    """Обе половины истории с готовыми рядами. Раскол — как у bots_honest.

    ГОТОВЫЙ ФИЛЬТР БОЛЬШЕ НЕ ЕДИНСТВЕННЫЙ ИСТОЧНИК: он зависит от генома, а
    part переживает смену генома (проверка устойчивости). Хранятся ряды aux,
    ...
    for name, a, b in (("train", 0, h), ("hold", h, n)):
        ...
        # build_filter, иначе измерит старый конфиг вместо нового.
        part["filt"] = build_filter(part, g)
        out[name] = part
    return out
...
def one_half(part, g, lev):
    ...
    ИТОГ ЗДЕСЬ НЕ СЛОЖНЫЙ, и раньше в этой строке было написано обратное.
    portfolio() складывает PnL на балансе, который стартует с e2.START=$20, а
    размер ставки от баланса НЕ зависит: маржа цикла жёстко e2.MARGIN=$5.
    Значит comp — это сумма результатов, делённая на неизменные $20.
    Просадка в той же функции считается от БЕГУЩЕГО ПИКА, который растёт по
    мере прибыли. Знаменатели разные, и пара «доход / просадка» на карточке
    сравнивает величины на разных базах: у LTC/final_wf обучение +104.68% при
    просадке 18.83% — это провал $8.48, то есть 42.4% от тех самых $20.
    Поэтому в webapp просадка показывается ещё и в долларах.
    ...
    """
    evs = []
    # фильтр строится из ТОГО ЖЕ g, с которым идёт прогон, а не берётся
    # готовым из part: именно через part проверка устойчивости теряла
    # возмущение половины генов (см. build_filter)
    r = bh.run_at(part["candles"], part["pre"], g, build_filter(part, g), lev,
