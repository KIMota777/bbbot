...
def funding_series(aux, lo=None, hi=None):
    """Ряд настоящих ставок фандинга В ДОЛЯХ для передачи в run5.

    ЗАЧЕМ ОТДЕЛЬНАЯ ФУНКЦИЯ. Правку знака фандинга пришлось прокидывать в
    шесть мест, и два из них были пропущены: график «прежней шкалы» и
    симуляция на странице бота считались ПЛОСКОЙ ставкой, пока карточка рядом
    считалась настоящими. Хуже того, правка молча изменила и эти два места:
    для шортового конфига плоская ставка из РАСХОДА стала ДОХОДОМ, и линия LTC
    подскочила на 17 п.п. как побочный эффект.

    Копия вызова в шести местах — и есть причина. Теперь единицы (в файлах
    ПРОЦЕНТЫ, делим на 100) и срез живут в одном месте.
    """
    f = aux.get("fund") if isinstance(aux, dict) else None
    if f is None:
        return None
    if lo is not None or hi is not None:
        f = f[lo or 0:hi if hi is not None else len(f)]
    return [(v / 100.0 if v is not None else 0.0) for v in f]
