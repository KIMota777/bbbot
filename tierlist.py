...
def tier_of(r):
    ...
    # ПРОСАДКА БЕРЁТСЯ ПЛАВАЮЩАЯ, а не по закрытым сделкам.
    #
    # portfolio считает провал только по закрытым циклам, и у флагманского
    # LTC/final_w это занижало холдоут в 2.6 раза: 3.8% против 9.7%. Порог в
    # 20% — это про деньги на счёте, а не про удобную метрику, поэтому здесь
    # используется та просадка, которую владелец видел бы на экране.
    # Закрытая остаётся в данных рядом, для сравнения.
    dd = max(tr.get("dd_float", tr["dd"]), ho.get("dd_float", ho["dd"]))
    if both_rob:
