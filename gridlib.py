...
BE_MARGIN = 0.0015       # безубыток = средняя + комиссии


def breakeven_stop(avg, stop, sgn, close):
    """Перенос стопа в безубыток. Возвращает (стоп, перенесён ли).

    Ограничение то же, что у trail_stop, и по той же причине. Триггер
    срабатывает по ХАЮ бара, а ордер выставляется на закрытии. Если бар успел
    вернуться ниже безубытка, стоп для лонга оказался бы ВЫШЕ рынка — биржа
    такой ордер не примет, а бэктест раньше спокойно «исполнял» по нему выход
    на следующем баре и засчитывал прибыль, которой не было.

    Замер на боевых конфигах показал, что это не редкий угол: у DOGE так
    происходило в 21% срабатываний, у LTC 20%, у SOL 25%. Именно на этих
    несуществующих выходах и держалась их бэктестовая доходность.

    Не перенесён = ждём бар, который ЗАКРОЕТСЯ по нужную сторону безубытка.
    """
    be = avg * (1 + sgn * BE_MARGIN)
    if (close < be) if sgn == 1 else (close > be):
        return stop, False
    better = (be > stop) if sgn == 1 else (be < stop)
    return (be if better else stop), True
