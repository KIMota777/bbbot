...
def main():
    ...
    for root, dirs, files in os.walk(REPO):
        ...
        # TEMP — черновики: туда пишут разовые скрипты и подсобные агенты.
        # Считать их продакшен-вызовами значит ронять сторож на ровном месте,
        # как и случилось 29.08.2026, когда там появилось пять черновиков.
    ...
    if os.path.exists(hist):
        ...
        # oi_chg стал 100% после 29.08.2026: глубину выборки ограничивала
        # наша пагинация (80 страниц), а не биржа — Bybit отдаёт открытый
        # интерес с февраля 2021. Лимит поднят, ряд закрывает окно целиком.
        check(abs(cov["oi_chg"] - 1.0) < 0.001
              and abs(cov["gold5"] - 0.428) < 0.001,
              f"покрытие окна: oi_chg {cov['oi_chg']:.1%}, "
              f"gold5 {cov['gold5']:.1%} — как написано в шапке")
        # Та же правка глубины: раньше на обучающей части открытый интерес
        # покрывал 46.1%, то есть ген oi_gate там почти не работал, и
        # половины сравнивались не на равных. Теперь 100%.
        check(abs(tr["oi_chg"] - 1.0) < 0.001
